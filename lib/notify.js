...
const EVENT_PRIORITY = ['closed', 'reopened', 'renamed', 'comment', 'assigned', 'milestone', 'labeled'];
 ...
 */
notify.issueEvent = async function ({ issue, event, dedupe, bodyShort, url }) {
	...
	const entry = { event: event, nid: nid, bodyShort: bodyShort, url: url || issue.url };
...
async function send(issue, entries) {
	const uids = await getRecipients(issue.pid);
	if (!uids.length) {
		return;
	}
	entries.sort((a, b) => EVENT_PRIORITY.indexOf(a.event) - EVENT_PRIORITY.indexOf(b.event));
