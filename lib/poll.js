...
async function pollIssues(config) {
	...
	let cursor = since;
	for (const issue of result.pages) {
		if (!issue || issue.pull_request) {
			continue;
		}
		cursor = maxTimestamp(cursor, issue.updated_at);
		await handleIssueSnapshot(config, issue);
	}
	await saveCursor({ pollIssuesSince: cursor, pollIssuesEtag: result.etag || '' });
}

// the poller has no event stream, so changes are derived by diffing the issue
// against the snapshot stored when it was last seen
async function handleIssueSnapshot(config, issue) {
	const pid = await common.getPidForIssue(config.repo, issue.number);
	if (!pid) {
		return;
	}
	...
	await notify.applySnapshot(pid, stored, issue);
}
...
async function handleComment(config, comment) {
	...
	const pid = await common.getPidForIssue(config.repo, number);
	if (!pid) {
		return;
	}
	const stored = await db.getObject(common.PID_KEY_PREFIX + pid);
